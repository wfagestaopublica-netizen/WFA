(function () {
      ...
      const GUIA_SCRIPT = [
        '========================================================================',
        'ESPECIFICAÇÃO DO SCRIPT — leia tudo antes de escrever',
        '========================================================================',
        '',
        'Você vai escrever um script para o sistema contábil da WFA Gestão',
        'Pública. Esse script lê relatórios em PDF que já foram carregados no',
        'sistema e devolve uma tabela, que aparece como planilha.',
        '',
        'RESPONDA APENAS COM O CÓDIGO.',
        'Sem explicação antes ou depois. Sem markdown. Sem cercas de código.',
        'Sem declarar função: escreva só o corpo, que será executado como se',
        'estivesse dentro de  function (dados) { ...aqui... }',
        'A última instrução precisa ser um return.',
        '',
        '------------------------------------------------------------------------',
        'O QUE CHEGA: a variável dados',
        '------------------------------------------------------------------------',
        '',
        'dados.documentos é a lista dos documentos que o usuário marcou na barra',
        'lateral, já lidos pelo sistema. Percorra-a sempre; pode haver mais de um.',
        '',
        'Todo documento tem:',
        '  tipo        "budget" (receita orçada), "collected" (receita',
        '              arrecadada), "committed" (despesa empenhada),',
        '              "consignacoes" (consignações da folha) ou',
        '              "desconhecido" (relatório que o sistema não sabe ler)',
        '  nome        nome do arquivo',
        '  titulo      nome do relatório por extenso',
        '  municipio   município do cabeçalho',
        '  total       soma de tudo o que foi lido, em número',
        '  linhas      registros já extraídos (vazio quando tipo = desconhecido)',
        '',
        'Conforme o tipo, também vem: exercicio (ano, nos relatórios de fonte) e',
        'competencia (mês de referência, na folha).',
        '',
        'Em budget, collected e committed, cada item de linhas tem:',
        '  chave       código da fonte, ex. "01 0500 0000 0000"',
        '  descricao   nome da fonte',
        '  valor       número',
        '  co          lista de códigos CO (só em collected)',
        '',
        'Em consignacoes, cada item de linhas tem:',
        '  chave       código da fonte que pagou',
        '  descricao   nome dessa fonte',
        '  codigo      código da verba',
        '  item        consignatária, ex. "EMPRESTIMO CAIXA I"',
        '  grupo       consignatária sem o algarismo romano: "EMPRESTIMO CAIXA"',
        '  ocorrencias quantidade de descontos, número',
        '  valor       número',
        '',
        'Em desconhecido, linhas vem vazio e o texto cru chega em:',
        '  textos      todas as linhas do PDF, na ordem, como texto',
        '  paginas     as mesmas linhas separadas por página',
        'Nesse caso é o script que precisa interpretar o relatório.',
        '',
        '------------------------------------------------------------------------',
        ...
        '  };',
        '',
        'colunas: lista de textos, na ordem em que devem aparecer.',
        'linhas: lista de listas, uma por linha da tabela.',
        '',
        'Devolva número como número (17263693.03), nunca como texto formatado.',
        'O sistema é que aplica o formato de dinheiro. Texto vai como está.',
        'Não crie linha de TOTAL: a planilha soma sozinha cada coluna numérica.',
        '',
        '------------------------------------------------------------------------',
        'REGRAS DO AMBIENTE — quebrar qualquer uma faz o script falhar',
        '------------------------------------------------------------------------',
        '',
        'O script roda isolado, sem acesso à página nem à internet.',
        'NÃO EXISTEM: document, window, localStorage, fetch, XMLHttpRequest,',
        'require, import, alert, console de página, nem acesso a arquivos.',
        'Não tente ler, salvar ou enviar nada: só receber dados e devolver a tabela.',
        'O script tem 5 segundos para responder; evite laços sobre laços sem',
        'necessidade.',
        'JavaScript moderno é aceito, mas prefira construções simples.',
        'Não invente campos além dos listados acima. Se um campo puder faltar,',
        'teste antes de usar.',
        '',
        '------------------------------------------------------------------------',
        ...
        '------------------------------------------------------------------------',
        '',
        '1. Se o pedido envolve um dos tipos conhecidos, use doc.linhas, que já',
        '   vem pronto. Não reprocesse o texto.',
        '2. Se o relatório chega como desconhecido, escreva a leitura a partir de',
        '   doc.textos, com expressão regular sobre cada linha.',
        '3. Se o pedido cruza documentos, junte por uma chave comum — normalmente',
        '   a fonte de recurso, que vem no mesmo formato em todos os relatórios.',
        '4. Em dúvida sobre o que agrupar, prefira a informação mais bruta e',
        '   deixe o usuário somar na planilha.',
        '',
        '------------------------------------------------------------------------',
        'EXEMPLO 1 — somar por consignatária (usa linhas já extraídas)',
        '------------------------------------------------------------------------',
        '',
        'var grupos = {};',
        'dados.documentos.forEach(function (doc) {',
        '  if (doc.tipo !== "consignacoes") return;',
        ...
        '',
        '------------------------------------------------------------------------',
        'EXEMPLO 2 — cruzar dois relatórios pela fonte de recurso',
        '------------------------------------------------------------------------',
        '',
        'var porFonte = {};',
        'dados.documentos.forEach(function (doc) {',
        '  doc.linhas.forEach(function (l) {',
        '    if (!porFonte[l.chave]) porFonte[l.chave] = { descricao: l.descricao };',
        '    porFonte[l.chave][doc.tipo] = (porFonte[l.chave][doc.tipo] || 0) + l.valor;',
        '  });',
        '});',
        'return {',
        '  colunas: ["Fonte", "Descrição", "Orçado", "Arrecadado", "Diferença"],',
        '  linhas: Object.keys(porFonte).sort().map(function (chave) {',
        '    var f = porFonte[chave];',
        '    var orcado = f.budget || 0;',
        '    var arrecadado = f.collected || 0;',
        '    return [chave, f.descricao, orcado, arrecadado, arrecadado - orcado];',
        '  })',
        '};',
        '',
        '------------------------------------------------------------------------',
        'EXEMPLO 3 — ler um relatório que o sistema não conhece',
        '------------------------------------------------------------------------',
        '',
        'var achadas = [];',
        'dados.documentos.forEach(function (doc) {',
        '  if (doc.tipo !== "desconhecido") return;',
        '  doc.textos.forEach(function (linha) {',
        '    var m = linha.match(/^(.+?)\\s+(-?[\\d.]+,\\d{2})$/);',
        '    if (!m) return;',
        '    achadas.push([m[1], Number(m[2].replace(/\\./g, "").replace(",", "."))]);',
        '  });',
        '});',
        ...
        '------------------------------------------------------------------------',
        '',
        '[ ] Respondi só com o código, sem markdown e sem declarar função?',
        '[ ] A última instrução é um return de { colunas, linhas }?',
        '[ ] Toda linha tem no máximo o mesmo número de células que colunas?',
        '[ ] Os valores numéricos são número, não texto com R$ e vírgula?',
        '[ ] Usei apenas campos que existem na lista de dados?',
        '[ ] Não usei document, window, fetch, localStorage nem import?',
        '',
        '========================================================================',
        'AGORA ESCREVA O SCRIPT PARA O PEDIDO ABAIXO',
        '========================================================================',
        '',
        'Pedido:'
      ].join('\n');
      ...
      if (botaoCopiarGuia) {
        botaoCopiarGuia.addEventListener('click', async function () {
          try {
            ...
            showToast('Especificação copiada.');
      ...
      function numeroDeOrdenacao(valor) {
        ...
        // o sinal vem antes do R$: "-R$ 1.234,56" precisa virar "-1.234,56"
        const texto = String(valor == null ? '' : valor).trim().replace(/^(-)?\s*R\$\s*/, '$1');
